...
- **Odd-sized images (FIXED):** Images with odd dimensions are now automatically resized to even dimensions (+1 pixel). This prevents illegal memory access in CUDA kernels.
- **Variable image sizes:** The demo app now supports processing images of different sizes in the same batch. Memory is dynamically managed for optimal performance.
